...
export default function OnvoBrandPage() {
  ...
  useEffect(() => {
    fetch('/api/tv-models')
      .then(res => res.json())
      .then(data => {
        setModels(data.filter((item: any) => item.brand === 'Onvo'));
        setLoading(false);
      });
  }, []);
  ...
  const onvoModels = models;
  const totalPages = Math.ceil(onvoModels.length / PAGE_SIZE);
  const pagedModels = onvoModels.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  // Arama sonuçları (en az 2 harf girildiyse, baştan eşleşenler)
  const filteredModels =
    search.length > 1
      ? onvoModels.filter((item) =>
    ...
    <div>
      <section className="bg-gradient-to-br from-blue-50 to-white py-16 pt-24 lg:pt-32">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl lg:text-5xl font-extrabold text-gray-900 mb-6 mt-4 text-center">Onvo TV Modelleri</h1>
          <p className="text-xl text-gray-600 mb-14">Onvo Televizyonunuz için Garantili Ekran Değişimi ve Profesyonel Tamir Hizmetleri. Modelinizi seçerek anında online fiyat teklifi alın veya uzman ekibimizle iletişime geçin.</p>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {pagedModels.map((model) => (
            <Link
              key={model.model}
              href={`/brands/onvo/${encodeURIComponent(model.model)}`}
        ...
        <div className="flex justify-center mt-10 gap-2 overflow-auto max-w-full whitespace-nowrap pb-4">
          ...
          <button
            onClick={() => router.push(`/brands/onvo?page=${page - 1}`)}
          ...
          {/* İlk sayfa */}
          <button
            onClick={() => router.push(`/brands/onvo?page=1`)}
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1)
            ...
            .map(i => (
              <button
                key={i}
                onClick={() => router.push(`/brands/onvo?page=${i}`)}
          ...
          {totalPages > 1 && (
            <button
              onClick={() => router.push(`/brands/onvo?page=${totalPages}`)}
